...
int solve(int maze[HEIGHT][WIDTH], int x, int y, int path[MAX_LEN][2], int move) {
    int success = 0, i;
    ...
    if (x < 0 || x >= WIDTH || y < 0 || y >= HEIGHT)
        ...
        //general cases: room[x][y] == UNVISITED
        maze[x][y] = VISITED;
        for (i = 0; i < 4 && !success; i++) {
            if(success = solve(maze, x + dx[i], y + dy[i], path, move + 1)) {
                //record a move
                path[move][0] = x + dx[i];
                path[move][1] = y + dy[i];
                return 1;
...
int main() {
    ...
    int maze[HEIGHT][WIDTH] = {
        {UNVISITED,     WALL,       WALL,       WALL,   WALL},
        {UNVISITED,     WALL,       WALL,       WALL,   WALL},
        {UNVISITED,     WALL,       WALL,       WALL,   WALL},
        {UNVISITED,     WALL,       WALL,       WALL,   WALL},
        {UNVISITED, UNVISITED, UNVISITED, UNVISITED,    EXIT}
    };
